...
function jumpToLogin() {//转跳到登录界面
    window.location.href = '/#/user/login?redirect=%2fgame%2ffib%2f'
}

function pharseScore(message) {
    message = message.data.slice(0, 10)
    $.each(message, function (index, item) {
        topNames[index] = item.user.username
    })
    $.each(message, function (index, item) {
        topScores[index] = item.score
    })
    $.each(message, function (index, item) {
        topRanks[index] = item.rank
    })
}
...
function newPhaseScore(message, score) { // 新方法
    message = message.data
    var best = message.find(function (value) {
        return value.user.username === username
    })
    if (best) {
    ...
    var current = message.find(function (value) {
    ...
    if (current)
        if (current.user.username === username)
            ...
            currentRank = current.rank + 1
    else
        currentRank = message.length + 1


    before = message.find(function (value) {
        return value.rank === bestRank - 1
    })
    after = message.find(function (value) {
        return value.rank === bestRank + 1
    })

    if (before) {
        playerBeforeName = before.user.username
        playerBeforeScore = before.score
    } else {
        playerBeforeName = ''
    }
    if (after) {
        playerAfterName = after.user.username
        playerAfterScore = after.score
    } else {
        playerAfterName = ''
    }
}
...
game.States.play = function () {
    this.create = function () {
        ...
        $.ajax({
            ...
            success: function (message) {
                newPhaseScore(message, self.score)
                self.updateText()
            }
        })
